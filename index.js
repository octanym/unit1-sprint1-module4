...
console.log(createMenuItem('sushi', 45, 'lunch'));
console.log(createMenuItem('burrito', 8, 'breakfast'));
...
const burger = {
  ...
  discount: function (customer) {
    return customer === 'teacher' || customer === 'student'
      ? burger.price * 0.75
      : burger.price * 0.9;
  },
};
...
function addReview(arr, name, rating, feedback) {
  let review = { name: name, rating: rating, feedback: feedback };
  arr.push(review);
  return arr;
}
...
function getReviewByIndex(arr, target) {
  const name = arr[target].name;
  const rating = arr[target].rating;
  const feedback = arr[target].feedback;
